...
def main():
    parser = argparse.ArgumentParser(
            description="Generate Pymol script for the visualisation of vector field based on data in cube format (for now only in 3D).",
            formatter_class=argparse.ArgumentDefaultsHelpFormatter
                    )
    parser.add_argument(
            "fn_cube",
            nargs=3,
            help="Three cube file for three dimensions."
            )
    parser.add_argument(
            "--coord",
            help="Which coordinate to scan (only one necessary, 1-->x, 2-->y, 3-->z).",
            default=2
            )
    parser.add_argument(
            "--scale",
            help="Cube scaling factor",
            default=1000
            )
    parser.add_argument(
            "--factor",
            help="Plot scaling factor",
            default=1.0
            )
    parser.add_argument(
            "--cutoff",
            help="Cutoff (default: 0.15)",
            default=0.15
            )
    parser.add_argument(
            "--head",
            help="Head width (default: 0.15)",
            default=0.15
            )
    parser.add_argument(
            "--head_length",
            help="Relative head length (default: 0.7)",
            default=0.7
            )
    parser.add_argument(
            "--sparse",
            help="Use every <sparse> grid point",
            default=1
            )
    parser.add_argument(
            "--c0",
            help="Which grid index of coord to start with",
            default=0
            )
    parser.add_argument(
            "--c1",
            help="Which grid index of coord to finish with",
            default=-1
            )
    parser.add_argument(
            "--f",
            help="Output file name",
            default='output.pymolrc'
            )
    parser.add_argument(
            "--color_rgb",
            nargs=3,
            help="Color code of the arrows (default: 0 0 0, i.e. black)",
            default=(0.0, 0.0, 0.0)
            )
...
#    print(pos1.shape, pos1.shape[coord], coord)
    for index in range(pos1.shape[coord-1]):
        if coord == 1:
            p0.append(pos1[index]/Angstrom2Bohr)
