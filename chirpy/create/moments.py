...
class OriginGauge_OLD(_CORE):
    ...
    def shift_origin_gauge(self, origins_aa, assignment=None,
        ...
        if 'd' in self._set:
            ...
            _Dd_p = [ed.shift_electric_origin_gauge(*_tup, cell_au_deg=_CELL)
                     for _tup in zip(_Qd, _Dd, _Rd, _O)]
            _D = _np.array([_Dd_p[_i].sum(axis=0) for _i in range(n_O)])

        # --- --- multipole

        # --- sum parts
        _M = _np.array([_Md_p[_i].sum(axis=0) for _i in range(n_O)])
        _C = _np.array([_Cd[_i].sum(axis=0) for _i in range(n_O)])
        _Q = _np.array([_Qd[_i].sum(axis=0) for _i in range(n_O)])
