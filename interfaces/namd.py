...
#TODO: unittests and other tests

def ReadNAMDOutput(fn_out,clean=False):
    fn  = " ".join(fn_out)
    p   = subprocess.Popen("egrep 'ENERGY:' %s | grep -v 'IMPRECISION'"%fn, stdout=subprocess.PIPE, shell=True)
    tmp = p.communicate()[0]
