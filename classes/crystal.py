...
class UnitCell(): #maybe all-in-one class
    def __init__(self,cell_aa_deg,**kwargs): #**kwargs for named (dict), *args for unnamed
        ...
        self.abc,self.albega = cell_aa_deg[:3],cell_aa_deg[3:]*_np.pi/180.
        self.abc_unitcell = _copy.deepcopy(self.abc)
    ...
    def propagate(self, data, multiply, priority=(0, 1, 2)):
        ...
        self.multiply = _np.array(multiply)

        #Distinguish between XYZData and VibrationalModes
        if hasattr(data,'pos_aa'): 
            cart_vec_aa = _get_cell_vec(self.cell_aa_deg, n_fields=data.n_fields, priority=priority)
            data.axis_pointer=1
...
#            new.data[:,:,:3] = np.remainder(new.data[:,:,:3],self.abc*self.multiply)
            ...
            cart_vec_aa = _get_cell_vec(self.cell_aa_deg, n_fields=3, priority=priority)
            new = _copy.deepcopy(data)            
            for iz,z in enumerate(multiply):
                tmp = _copy.deepcopy(new)            
                for iiz in range(z-1):
                    tmp.pos_au[:,:] += cart_vec_aa[_np.newaxis,iz]*constants.l_aa2au
                    new += tmp
