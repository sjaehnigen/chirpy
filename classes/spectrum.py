...
class VCDSampling(): #later: merge it with itertools (do not load any traj data before the actual processing)
    def __init__(self,path,**kwargs): #**kwargs for named (dict), *args for unnamed
        fn_m = _os.path.join(path,'MOMENTS')
        fn_t = _os.path.join(path,'TRAJECTORY')
        self.path = path
        self.n_moms  = vcd_utils.NumberOfMoleculesFromMomentFile(fn_m) #later replace this by own routine     
        buf = _np.loadtxt(fn_m, usecols=(1,2,3,4,5,6,7,8,9))
        ...
        buf = _np.loadtxt(fn_t, usecols=(1,2,3,4,5,6))
    ...
    def __iadd__(self,other):
        self.moments = _np.concatenate((self.moments,other.moments),axis=0)
        self.trajectory = _np.concatenate((self.trajectory,other.trajectory),axis=0)     
    ...
    def unravel(self): #better use ther iterlist routines of Arne
        self.r_aa, self.c_aa, self.m_aa = tuple(_np.rollaxis(self.moments,axis=2)) 
    #    return tuple(_np.rollaxis(self.moments,axis=2))
