...
class Domain3D():
    ...
    #    if not _np.allclose(s_field.shape,self.grid_shape):
    ...
    def map_vector(self,v3):
        ...
        ind = self.indices
        v3_field[:,ind[0],ind[1],ind[2]] = self.weights[_np.newaxis,:]*v3[:,_np.newaxis]
        return v3_field

    #def integrate_vector_field():
        #if not _np.allclose(v_field.shape[0],self.grid_shape):
...
#        self.voxel     = _np.dot(self.cell_au[0],_np.cross(self.cell_au[1],self.cell_au[2]))
...
#                fn = tempfile.TemporaryFile(dir='/tmp/')
            transfer = _np.memmap(fn,dtype='float64',mode='w+',shape=(self.n_domains,self.n_domains,3,)+self.shape)
            ...
            for i_d in range(self.n_domains):
                data = self.fd[i_d]
                intg = _ScalarField(fmt='manual',data=data.sum(axis=0),**vars(self)).integral()
                print(intg)
#            tmp = 
#        if self._tmp: 
