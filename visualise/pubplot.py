...
def multiplot(ax, x_a, y_a, **kwargs):
    ...
    if ylim is None: #add routine for pile_up option
        ...
        if _exp is not None:
            ylim = (min(ylim[0], np.amin(_e[_slce])), max(ylim[1], np.amax(_e[_slce])))
    ...
    if pile_up:
        if not np.allclose(np.unique(x_a),  x_a[0]):
            raise ValueError('pile_up argument requires identical x_a content!')
    ...
    if stack:
        ...
        LB = [pub_label(ax, color=_c, X=np.mean(xlim), Y=np.mean(ylim), alpha=_al) for _c, _al in zip(color_a, alpha_a)] \
...
def histogram(ax_a, data_a, **kwargs): #i.c.to multiplot,  this routine needs a list of ax
    ...
    if sum_one:
        ...
        weights_a = kwargs.get('weights_a', [np.ones_like(_d) for _d in data_a])
    for key in ['color_a', 'color', 'alpha_a', 'alpha', 'bool_a', 'weights', 'weights_a', 'sum_to_one', 'edges', 'facecolor', 'edgecolor', 'bins', 'sep', 'ylim']: # this is unsatisfactorily clumsy; get routine that read AND deletes argument
        try:
            del kwargs[key] #otherwise hist may complain (this is because we pass through kwargs)
        except:
            pass
