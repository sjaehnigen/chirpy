...
def cubeWriter(fn, comments, numbers, pos_au, cell_vec_au, data, append=False, origin_au=[0.0, 0.0, 0.0]):
    ...
    with open(fn, fmt) as f:
        outbuffer = _assemble_cube_file(comments[0], comments[1], numbers, pos_au, cell_vec_au, data, origin=origin_au)
        f.write(outbuffer)

def _assemble_cube_file(comment1, comment2, numbers, coords, cell, data, origin):
